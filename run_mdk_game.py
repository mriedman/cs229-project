...
from decision_func_agent import DecisionFunctionAgent
from rl_agent import RLDecisionFunctionAgent
...
def main(args):
    agent_dict = {'advhm': AdvancedHumanMistakeAgent, 'advh': AdvancedHumanAgent,
                  'mdk': TrainingObserver, 'dfa': DecisionFunctionAgent, 'rla': RLDecisionFunctionAgent}
    seed(1)
    score_list = []
    agents = []
    args['print'] = 0
    ...
    for _ in range(args['num_rounds']):
        ...
        if _ == args['num_rounds'] - 1:
            args['print'] = 1
        score_list.append(run_game(args, agents, verbose=args['verbose'], train=False))
        for agent in agents:
            if type(agent) == RLDecisionFunctionAgent:
                agent.get_score(score_list[-1])
            agent.reset(args)
...
for i in [('-p', 'players', 2, int), ('-c', 'colors', 5, int), ('-r', 'rank', 5, int), ('-hs', 'hand_size', 5, int),
          ...
          ('-al', 'alpha', 0.05, float), ('-a2', 'alpha2', 0, float), ('-a3', 'alpha3', 0, float),
          ('-df', 'decision_func', 0, str)]:
    p.add_argument(i[0], dest=i[1], default=i[2], type=i[3])
p.add_argument('-a', dest='agents', default=['advh', 'advh'], nargs='*')
args = vars(p.parse_args(sys.argv))

res = np.array(main(args))
